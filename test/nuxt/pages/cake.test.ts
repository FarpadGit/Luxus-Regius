...
import Cake from "@app/pages/cake.vue";
import {
  mockComponent,
  mockNuxtImport,
  mountSuspended,
...
describe("Cake Page", async () => {
  ...
  async function mountComponent() {
    component = await mountSuspended(Cake);
  }
  ...
  it("should display a hero section with a text heading, background image and a ReactiveMenu component", async () => {
    ...
    expect(img.attributes()["src"]).toMatch(/hero.*cake/i);
    const heroText = hero.find("h1#hero-text");
    expect(heroText.exists()).toBe(true);
    expect(heroText.text()).toMatch(/torta|torták/i);
