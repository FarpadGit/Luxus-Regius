...
import Cocktail from "@app/pages/cocktail.vue";
import {
  mockComponent,
  mockNuxtImport,
  mountSuspended,
...
describe("Cocktail Page", async () => {
  ...
  async function mountComponent() {
    component = await mountSuspended(Cocktail);
  }
  ...
  it("should display a hero section with a text heading, background image and a ReactiveMenu component", async () => {
    ...
    expect(img.attributes()["src"]).toMatch(/hero.*cocktail/i);
    const heroText = hero.find("h1#hero-text");
    expect(heroText.exists()).toBe(true);
    expect(heroText.text()).toMatch(/koktél/i);
