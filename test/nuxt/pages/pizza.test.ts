...
import Pizza from "@app/pages/pizza.vue";
import {
  mockComponent,
  mockNuxtImport,
  mountSuspended,
...
describe("Pizza Page", async () => {
  ...
  async function mountComponent() {
    component = await mountSuspended(Pizza);
  }
  ...
  it("should display a hero section with a text heading, background image and a ReactiveMenu component", async () => {
    ...
    expect(img.attributes()["src"]).toMatch(/hero.*pizza/i);
    const heroText = hero.find("h1#hero-text");
    expect(heroText.exists()).toBe(true);
    expect(heroText.text()).toMatch(/pizza|pizzák/i);
