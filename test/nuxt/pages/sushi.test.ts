...
import Sushi from "@app/pages/sushi.vue";
import {
  mockComponent,
  mockNuxtImport,
  mountSuspended,
...
describe("Sushi Page", async () => {
  ...
  async function mountComponent() {
    component = await mountSuspended(Sushi);
  }
  ...
  it("should display a hero section with a text heading, background image and a ReactiveMenu component", async () => {
    ...
    expect(img.attributes()["src"]).toMatch(/hero.*sushi/i);
    const heroText = hero.find("h1#hero-text");
    expect(heroText.exists()).toBe(true);
    expect(heroText.text()).toMatch(/sushi/i);
