...
import Weekly from "@app/pages/weekly.vue";
import {
  mockComponent,
  mockNuxtImport,
  mountSuspended,
...
describe("Weekly Page", async () => {
  ...
  async function mountComponent() {
    component = await mountSuspended(Weekly);
  }
  ...
  it("should display a hero section with a text heading, background image and a ReactiveMenu component", async () => {
    ...
    expect(img.attributes()["src"]).toMatch(/hero.*weekly/i);
    const heroText = hero.find("h1#hero-text");
    expect(heroText.exists()).toBe(true);
    expect(heroText.text()).toMatch(/heti/i);
